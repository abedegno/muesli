"""Tests for data: URL decoding (the audio-fetch hardening)."""
import base64

import pytest

from whisper_app.transcribe import _decode_data_url


def test_base64_data_url_round_trips():
    payload = base64.b64encode(b"audio-bytes").decode()
    assert _decode_data_url(f"data:audio/wav;base64,{payload}") == b"audio-bytes"
