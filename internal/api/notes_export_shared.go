...
func renderNoteExport(note model.Note, parts noteExportParts, format string, options noteexport.Options) ([]byte, string, string, error) {
	filename := noteexport.SlugifyFilename(note.Title)
	switch strings.ToLower(format) {
	case "txt":
		return []byte(noteexport.RenderNoteText(note, parts.summarySections, parts.segments, parts.aliases, options)),
			"text/plain; charset=utf-8", filename + ".txt", nil
	case "docx":
		rendered, err := noteexport.RenderNoteDocx(note, parts.summarySections, parts.segments, parts.aliases, options)
		if err != nil {
			return nil, "", "", err
		}
		return rendered,
			...
			filename + ".docx", nil
	case "pdf":
		rendered, err := noteexport.RenderNotePdf(note, parts.summarySections, parts.segments, parts.aliases, options)
		if err != nil {
			return nil, "", "", err
		}
		return rendered, "application/pdf", filename + ".pdf", nil
	default:
		return []byte(noteexport.RenderNoteMarkdown(note, parts.summarySections, parts.segments, parts.aliases, options)),
