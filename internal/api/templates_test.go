...
func TestTemplatesCRUDAPI(t *testing.T) {
	...
	body := map[string]any{"name": "Standup", "sections": []map[string]string{{"heading": "Overview", "instruction": "Summarise it."}}}
	...
	if l.Code != http.StatusOK || !strings.Contains(l.Body.String(), "Standup") {
		t.Fatalf("list=%d body=%s", l.Code, l.Body.String())
	}

	// rename ok
	u := doJSON(t, srv, http.MethodPut, "/api/templates/"+created.ID, map[string]any{"name": "Standup2", "sections": body["sections"]}, hdr)
	if u.Code != http.StatusOK {
		t.Fatalf("update=%d body=%s", u.Code, u.Body.String())
	}
	// duplicate name → 409
	_ = doJSON(t, srv, http.MethodPost, "/api/templates", map[string]any{"name": "Dupe", "sections": body["sections"]}, hdr)
	d := doJSON(t, srv, http.MethodPost, "/api/templates", map[string]any{"name": "dupe", "sections": body["sections"]}, hdr)
	if d.Code != http.StatusConflict {
		t.Fatalf("dup: want 409 got %d", d.Code)
	}
	// bad (no sections) → 400
	b := doJSON(t, srv, http.MethodPost, "/api/templates", map[string]any{"name": "X", "sections": []any{}}, hdr)
	if b.Code != http.StatusBadRequest {
		t.Fatalf("bad: want 400 got %d", b.Code)
	}
	// updating a non-existent / built-in id → 404
	nf := doJSON(t, srv, http.MethodPut, "/api/templates/00000000-0000-0000-0000-000000000000", map[string]any{"name": "Y", "sections": body["sections"]}, hdr)
